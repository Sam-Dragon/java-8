...
public class MaxOfSalaryOfEmployee {
    ...
    public static void main(String[] args) {
        OptionalInt maxByMapToIntAndMax = Dish.menu().stream().mapToInt(Dish::getCalories).max();
        System.out.println("Max of Calories [mapToInt & max] :: " + maxByMapToIntAndMax.getAsInt());

        Optional<Integer> maxByMapAndMaxWithComparator = Dish.menu().stream().map(Dish::getCalories)
        ...
        Optional<Integer> maxWithReduce = Dish.menu().stream().map(Dish::getCalories).reduce(Integer::max);
        System.out.println("Max of Calories [map & reduce] :: " + maxWithReduce.get());

        int maxWithReduceWithDefaultValue = Dish.menu().stream().map(Dish::getCalories).reduce(0, Integer::max);
        System.out.println("Max of Calories [map & reduce with default value] :: " + maxWithReduceWithDefaultValue);

        List<Integer> maxWithSortAndLimit = Dish.menu().stream().map(Dish::getCalories)
        ...
        Optional<Integer> maxWithFindFirst = Dish.menu().stream().map(Dish::getCalories)
        ...
        IntSummaryStatistics maxWithSummaryStatistics = Dish.menu().stream().mapToInt(Dish::getCalories).summaryStatistics();
